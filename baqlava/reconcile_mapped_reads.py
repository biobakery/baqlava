...
def process_baqlava_output(sysargv1, nuc_tax, prot_tax):
    ...
    df7 = df6.groupby(['Species'], as_index=False).mean()[['Species', base]].copy()
    df8 = df7[df7[base] != 0]
    df9 = df8.copy()
    trans = df9['Species']
    df9['subset'] = trans + "|translated"
    #now merge together and format results:
    df10 = pd.concat([df4,df9])
    df11 = df10.groupby(['Species'], as_index=False).sum()#[['Species', base]]
    df12 = pd.concat([df10, df11], sort=False)
    df12[['subset']] = df12['subset'].fillna(df12['Species'])
    return df12.sort_values(by=['subset'], ascending=[True])[['subset', base]].rename(columns={'subset':'# Taxon'})
